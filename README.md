...
## Ghi đè file `charsets.jar` trong spring java
 - Sử dụng [file này](https://github.com/LandGrey/spring-boot-upload-file-lead-to-rce-tricks/blob/main/release/charsets.jar) để test xem có bị không. Nếu trigger thành công ta sẽ nhận response có status code 500, còn nếu failed ta sẽ nhận response có status code 406.
 - các câu lệnh hỗ trợ build file jar:
   ```
   mkdir out
...
- Với hàm `ip_address` trong python thì sử dụng ký tự `%` để bypass đối với IPv6
